...
interface ImageUploadProps {
  ...
  uploadUrl: string;
  resourceType: "image" | "video" | "documents" | "others";
  onUploadSuccess: (dto: { url: string; _id: string }) => Promise<void> | void;
...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  onUploadError,
  maxSizeMB = 5,
  text = "Drag and drop your image here, or click to browse",
  ...
  const handleFileSelect = useCallback(
    (file: File | undefined) => {
      ...
      if (!acceptedFormats.includes(file.type)) {
        toast.error(
          `Invalid format. Accepted formats: ${acceptedFormats.join(", ")}`,
        );
        return;
      }
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const _id = uploadRes.data.id;
        // move this logic outside has a callback function.
        onUploadSuccess({
          url,
          _id,
        });
